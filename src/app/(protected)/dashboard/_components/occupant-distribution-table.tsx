...
export function OccupantDistributionTable() {
  const distributionData = useMemo(() => {
    ...
    return AREAS.map((area) => {
      ...
      // Count by gender
      const maleCount = areaBeds.filter(
        (b) => b.occupant?.gender === "L"
      ).length;
      const femaleCount = areaBeds.filter(
              ...
              {distributionData.map((data) => (
                <TableRow key={data.area.id} className="hover:bg-muted/30">
                  ...
                  <TableCell className="text-center">
                    <Badge
                      variant="outline"
                      className="bg-sky-50 text-sky-700 border-sky-200 dark:bg-sky-900/30 dark:text-sky-400"
                    >
                      {data.male}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-center">
                    <Badge
                      variant="outline"
                      className="bg-pink-50 text-pink-700 border-pink-200 dark:bg-pink-900/30 dark:text-pink-400"
                    >
                      {data.female}
                    </Badge>
                  </TableCell>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {data.topCompanies.map(([company, count]) => (
                        <Badge
                          ...
                          title={company}
                        >
                          {company.replace("PT. ", "").substring(0, 15)}... (
                          {count})
                        </Badge>
                      ))}
                    </div>
